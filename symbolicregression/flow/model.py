"""ELF continuous flow-matching denoiser.

Ported from ELF-pytorch_elf/src/modules/model.py into the symbolic-regression
package. Changes vs upstream:
  - import path: ``modules.layers`` -> ``.layers``
  - ``vocab_size`` is required (no default); pass ``len(equation_words)``.
"""
...
class ELF(nn.Module):
    ...
    def forward(
        ...
        prefix length is fixed at construction, so pass ``self_cond_cfg_scale``
        whenever ``num_self_cond_cfg_tokens > 0`` (see ELF-pytorch train_step.py).
        """
        B = x.shape[0]
